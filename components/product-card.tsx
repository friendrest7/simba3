  ...
  return (
    <article className={`group min-w-0 rounded-xl border border-transparent p-2 transition duration-300 hover:-translate-y-1 hover:border-line hover:bg-canvas hover:shadow-[0_14px_35px_rgba(0,0,0,.08)] ${soldOut ? "opacity-75" : ""}`}>
      <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-[#f2f2f0]">
        ...
        <span className={`absolute bottom-3 left-3 rounded-sm px-2.5 py-1 text-[10px] font-black uppercase text-white ${soldOut ? "bg-[#555]" : "bg-[#16865c]"}`}>{soldOut ? t("unavailableHere") : `${branchStock} ${t("inStock")}`}</span>
        <button type="button" onClick={() => toggleSavedProduct(product.id)} className={`absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-white/95 text-[#171719] shadow-sm transition hover:scale-105 focus-visible:opacity-100 ${saved ? "text-brand opacity-100" : "opacity-80 sm:opacity-0 sm:group-hover:opacity-100"}`} title={saved ? t("removeSaved") : t("saveProduct")} aria-pressed={saved}><Heart className={`h-4 w-4 ${saved ? "fill-current" : ""}`} /></button>
      </div>
      <div className="pt-4">
        ...
        <p className="mt-1 text-xs text-muted">{t("by")} {product.seller}</p>
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          {quantity > 0 ? (
            ...
            <button type="button" disabled={soldOut} onClick={() => addToCart(product)} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-brand text-white transition hover:scale-105 hover:opacity-90 disabled:cursor-not-allowed disabled:bg-[#777]" title={soldOut ? t("soldOut") : t("addToCart")}><Plus className="h-4 w-4" /></button>
          )}
        </div>
      </div>
    </article>
