        ...
        <div className="mx-auto flex max-w-[1600px]">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex overflow-x-auto border-b border-black/10 bg-white dark:border-white/10 dark:bg-[#1b1c20] lg:hidden">
              {menu[role].map(([Icon, label]) => (
                <button key={String(label)} type="button" onClick={() => setActiveTool(String(label))} className={`flex shrink-0 items-center gap-2 px-4 py-3 text-xs font-bold border-b-2 transition ${activeTool === String(label) ? "border-brand text-brand" : "border-transparent text-gray-500"}`}>
