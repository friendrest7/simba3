      ...
      <div className="mx-auto grid max-w-[1500px] gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[1.5fr_1fr_1fr_1fr] lg:px-10">
        <div className="max-w-sm">
          ...
          <div className="mt-6 flex gap-2">
            {socialLinks.map(({ label, href, Icon }) => (
              <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="grid h-10 w-10 place-items-center rounded-md border border-white/15 text-white/70 transition hover:border-brand hover:bg-brand hover:text-white">
        ...
        {groups.map((group) => (
          <div key={group.title}>
            ...
            {group.links.map(([label, href]) => <Link key={label} href={href} className="mb-3 block text-sm text-white/55 hover:text-white">{label}</Link>)}
          </div>
        ))}
      </div>
