...
function OrderCard({ order, recurringOrders, frequency, setFrequency, buyAgain, makeRecurring }: { order: Order; recurringOrders: RecurringOrder[]; frequency: "weekly" | "biweekly" | "monthly"; setFrequency: (f: "weekly" | "biweekly" | "monthly") => void; buyAgain: (o: Order) => void; makeRecurring: (o: Order) => void }) {
  ...
  return <article className="dashboard-card rounded-xl">
    ...
    {order.status === "packed" || order.status === "on_the_way" ? <div className="mt-5 rounded-lg border border-line p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase text-brand">Live delivery view</p><p className="mt-1 text-xs text-gray-500">See the current driver route for this order.</p></div></div><div className="mt-4"><DeliveryMap compact branchId="kigali-kic" driverId="drv-101" /></div></div> : null}
